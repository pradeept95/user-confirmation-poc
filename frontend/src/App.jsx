...
function App() {
  ...
  const cancelTask = async () => {
    ...
    if (sessionId) {
      await fetch(`http://localhost:8000/cancel-task/${sessionId}`, { method: 'POST' })
    }
  ...
  const handleInputSubmit = () => {
    if (ws) ws.send(JSON.stringify({ type: 'user_input', values: inputValues }))
    setInputFields(null)
  }

  // Show submitted values after task completes
  useEffect(() => {
    if (taskStatus === 'closed' || taskStatus === 'cancelled' || taskStatus === 'not confirmed') {
      setInputFields(null)
      setShowConfirm(false)
    }
  }, [taskStatus])
  ...
  // Listen for task completion (simulate with WebSocket close)
  useEffect(() => {
    if (ws) {
      ws.onmessage = (event) => {
  ...
  return (
    <div style={{ padding: 32 }}>
      ...
      <button onClick={startTask} disabled={taskStatus === 'running'}>Start Task</button>
      <button onClick={cancelTask} disabled={taskStatus !== 'running'}>Cancel Task</button>
